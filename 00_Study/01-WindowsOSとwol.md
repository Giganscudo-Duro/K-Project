...
- 電源オフ（ハイブリッドシャットダウン）
    - 設定手順  
        [コントロールパネル > ハードウェアとサウンド > 電源オプション > 電源ボタンの動作を選択する] を選択する。  
        [現在利用可能ではない設定を変更します] をクリックして設定変更可能にし、 [シャットダウン設定] の項にある[高速スタートアップを有効にする（推奨）] のチェック ボックスをオンにし、[変更の保存] をクリックする。  
    - 実施手順  
...
- 電源オフ（クラシックシャットダウン）
    - 設定手順  
        [コントロールパネル > ハードウェアとサウンド > 電源オプション > 電源ボタンの動作を選択する] を選択する。  
        [現在利用可能ではない設定を変更します] をクリックして設定変更可能にし、 [シャットダウン設定] の項にある[高速スタートアップを有効にする（推奨）] のチェック ボックスをオフにし、[変更の保存] をクリックする。  
    - 実施手順  
        [スタート] を 選択し 、[電源 > シャットダウン] を選択する。
...
- 補足１）ネットワークアダプタの設定変更
    ...
    - 設定手順  
        [デバイスマネージャー] を起動し、[ネットワークアダプター]のツリーを展開する。
        [使用しているネットワークアダプター名]を右クリックし、プロパティを開く。
        [詳細設定]タブに切り替えて「Wake on Magic Packet」関連の項目の値を「Enabled(有効)」に変更する。
...
- 補足２）ネットワークアダプタの電源管理設定変更
    ...
    - 設定手順  
        [デバイスマネージャー] を起動し、[ネットワークアダプター]のツリーを展開する。
        [使用しているネットワークアダプター名]を右クリックし、プロパティを開く。
        [電源の管理]タブに切り替えて、以下の項目についてチェックを入れる。
