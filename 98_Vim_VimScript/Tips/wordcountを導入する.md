...
https://ushumpei.hatenablog.com/entry/2017/12/09/214116


# 導入方法

1. 導入先となるパスに `wordcount.vim` を配置
    - Unix 系の場合
        `~/.vim/plugin/wordcount.vim`

    - Windows 系の場合（GNUpack）
        GNUpackの場合、ファイルの権限回りが厄介で `$HOME/vimfiles/plugin` にファイルを作れなかったり、権限をいじれなかったりする。
        そのため以下のディレクトリに配置して、gVim で使えるようにする。
        - `gnupack-pretest_devel-2018.11.25\app\vim\plugins\kaoriya\plugin\wordcount.vim`
...
2. `.vimrc` に追加
    ...
    - Windows 系の場合（GNUpack）
        前述のとおり、gVim で利用できるよう `_gvimrc` に設定を加える。
        - `$HOME\_gvimrc`
            ```
